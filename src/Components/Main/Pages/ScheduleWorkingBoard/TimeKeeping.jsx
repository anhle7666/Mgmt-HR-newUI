...
const TimeKeeping = () => {
    ...
    function findShift(timegroup) {
        const { IDEmployee, date } = timegroup;
        ...
        const filteredSchedule = Schedule.filter(
            (shift) => shift.title.includes(IDEmployee) && new Date(shift.start).getDate() === newDate.getDate(),
        );

        console.log(filteredSchedule);
        const time = filteredSchedule[0].title.split("-");
        return time[1];
    }
    ...
    const result = timekeeping.reduce((acc, curr) => {
        const existing = acc.find(
            (item) =>
                // console.log(item)
                item.IDEmployee === curr.IDEmployee && item.date,
            // item.date.toDateString() === curr.time.toDateString(),
        );
